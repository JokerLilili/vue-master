    ...
    <el-col :span="24" class="main">
      <aside :class="collapsed?'menu-collapsed':'menu-expanded'">
        ...
        <el-menu :default-active="$route.path" class="el-menu-vertical-demo" @open="handleopen" @close="handleclose"
          ...
          <template v-for="(item,index) in routeLists">
        ...
        <ul class="el-menu el-menu-vertical-demo collapsed" v-show="collapsed" ref="menuCollapsed">
          <li v-for="(item,index) in routeLists" :key="item.name" class="el-submenu item">
            <!--非单节点route渲染-->
...
export default {
  ...
  data () {
    return {
      ...
      sysUserName: '',
      sysUserAvatar: '',
      role: sessionStorage.getItem('user') || this.$store.getters.getRoles,
      routeList: [],
      routeLists: [],
      routeKey: true,
      form: {
  ...
  created () {
    this.routeLists = this.$router.options.routes.filter(this.getRouter)
    this.routeLists.map(item => {
      let thatchild = item.children.filter(this.getRouterChildren)
      item.children = thatchild
    })
  },
  methods: {
    keychange () {
    ...
    getRouter (item) {
      return item.path === '/' && item.meta.indexOf(JSON.parse(this.role).username) > -1
    },
    getRouterChildren (item) {
      return item.meta.indexOf(JSON.parse(this.role).username) > -1
    },
    onSubmit () {
      console.log('submit!')
    },
